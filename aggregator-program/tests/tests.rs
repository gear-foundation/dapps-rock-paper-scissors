...
use rps_aggregator::entities::*;
use rps_io::{GameConfig, InitConfig};

#[test]
fn common() {
    let sys = System::new();
    let aggregator = init_aggregator(&sys);
    ...
    let result = aggregator.send(
        3,
        AggregatorAction::Create {
            init_payload: config.encode(),
        },
    );
    ...
    let AggregatorStateReply::GeneratedPrograms(programs) = aggregator
        .meta_state(AggregatorState::GeneratedPrograms)
        .unwrap();
...
fn wrong_init() {
    ...
    // println!("{:?}", programs.first().unwrap());
    assert_eq!(programs.len(), 0);
    assert!(result.main_failed());
}

fn init_aggregator(sys: &System) -> Program {
    let aggregator_code_hash =
        sys.submit_code("../target/wasm32-unknown-unknown/release/rock_paper_scissors.opt.wasm");
    let aggregator = Program::current(sys);
    let res = aggregator.send(100, aggregator_code_hash);
